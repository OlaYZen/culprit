...
| **Patch notes** | What changed in this host, straight from the checkout's own commit history: every commit grouped under the version `version.json` carried after it, newest first, with a Features / Fixes filter. Nothing is written by hand and nothing can go stale; a host without a checkout (the container image) says so instead of showing an empty list |
