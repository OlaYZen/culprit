...
.venv/bin/python tools/record_demo.py --rename Jellyfin=media --alias <user>=sam   # demo: record + scrub
                                           # fixtures from a live host into web/demo/data/
.venv/bin/python tools/build_demo.py       # demo: static site into dist/demo/ (serve with any
                                           # static server; any real host also answers ?demo)
```
...
**Demo mode (`web/js/demo/`, the GitHub Pages site).** app.js dynamically imports `demo/index.js` when `<html data-demo="1">` (set by the build) or `?demo` is present, and *before* `store.connect()`; the module replaces `window.fetch` for `/api/*` and `window.EventSource` with an in-browser stand-in, so every view runs the code it runs against a host. `data.js` loads the fixtures from `web/demo/data/` (relative to `import.meta.url`, so the same files work under `/assets/demo/data/` on a host and under a Pages sub-path) and time-shifts every epoch under a known key by `now - recorded_at`; `world.js` keeps the fleet alive (fast-tier jitter around the recorded values, a per-minute rollup into the history series, the scripted `psi_cpu` incident on `media` with an ffmpeg row inside the jellyfin container, an offline `dev`, nice/throttle/terminate state, a verdict watch that follows `verdict.py`'s rules, expectations with the same window matching as `expect.py`); `routes.js` answers every route the views call with main.py's shapes and error texts. Fixtures come from **`tools/record_demo.py`** (records a live host's reads and scrubs them -- `--rename node=new`, `--alias user=alias`, `--replace old=new`, public IPs → 203.0.113.x, MACs/link-local v6/ids hashed; read the output before committing it) and live in `web/demo/data/` (un-ignored explicitly, since `.gitignore` ignores every `data/`). **`tools/build_demo.py`** writes `dist/demo/` (assets under `assets/`, root paths made relative, the flag set); `.github/workflows/demo.yml` builds and deploys it on push (Pages source must be "GitHub Actions"). Rules: the mock lives on `main` behind the flag -- there is no hand-maintained demo branch to rot; a payload rename that touches `check_contract.py` must also touch `world.js` where it synthesises that field; the demo module never uses `innerHTML`; nothing in the fixtures or the module may be presented as live -- the sticky banner is not optional.
