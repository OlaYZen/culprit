...
Nobody-at-fault findings (`lag.py`): `cpu_steal`, `thermal_throttle` (from `cpu.thermal`, sysfs throttle counters), `swap_slow` (swap activity while `memory.swap_rotational`), and `stuck_procs` when every wchan is an NFS/SMB/FUSE/Ceph wait carry `external: True` + `blame`; external findings list no culprits (the D-state victims are the one exception, flagged `victims`). Keep that rule: ranking processes under a cause they did not create is invention.
