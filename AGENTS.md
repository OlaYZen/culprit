...
.venv/bin/python tools/check_map.py         # offline (~2s): the fleet map from synthetic nodes (edge
                                           # direction, findings joined, client-side signs, chains,
                                           # blast radius) plus this machine's real socket table
...
- **The Map** (`fleetmap.py`, host-only, read-time): `FleetMap.build` joins every online node's `network_detail.sockets.established` (each connection now carries `name`, `unit`, `tx_queue`, `rx_queue` and, with `ss`, `rtt_ms`, `rtt_min_ms`, `retrans`, `send_bytes_sec`, `recv_bytes_sec` from `network._ss_established`, read passively; `sockets.tcp_info` / `tcp_info_reason` say whether `ss -ti` was available; `sockets.per_process` is the per-process sum) with the nodes' interface addresses and `ports` listeners: an edge per (client node, client process name, target node, port), counted once from the client side (a connection whose local port is one of the node's own listeners is the server side of someone else's edge), loopback and self-addressed connections skipped, unknown addresses grouped as `external`. `_finish_edge` attaches the target's warn/critical findings that name the listener's pid, unit or port or are machine-wide (`psi_*`, `cpu_*`, `memory*`, `disk_*`, `swap_*`, `stuck*`), and the client kernel's own `signs` (a send queue with bytes, `retrans_sec` from two builds, RTT ≥200 ms or ≥5× the connection's minimum); a `turned_away` listener makes the edge critical. `_chain` states an edge under a finding as *depends on a service under a finding* and as *the client is feeling it* only with client-side signs; an unattributed client with no signs is never a chain. `radius(node, pid)` is the graph read backwards (inbound edges to the pid or its unit, outbound from it); `GET /api/map` and `GET /api/map/radius` serve both, the process dialog fetches the radius, `views/map.js` draws the graph on a canvas. Never send a probe from the Map: everything on an edge is passive.
